...
const createChatRoom = async (request, name) => {
  const formData = new URLSearchParams();
...
const joinChatRoom = async (request, chatRoomId) => {
  const response = await request.post(
    `${apiBaseUrl}/api/chat-rooms/${chatRoomId}/join`
  );
...
test('입장 시점 이전에 작성된 메시지는 신규 입장 사용자에게 표시되지 않음', async ({ browser, request }) => {
  ...
  await createChatRoom(request, roomName);
...
test('재입장 시 부재 중 메시지는 표시되지 않음', async ({ browser, request }) => {
  ...
  const creator = await createUser(request, creatorNickname);
  const chatRoom = await createChatRoom(request, roomName);  // creator 세션으로 생성
  const joiner = await createUser(request, joinerNickname);  // 이후 세션 = joiner

  // joiner 최초 입장 후 퇴장 (API)
  await joinChatRoom(request, chatRoom.id);  // joiner 세션
  const leaveRes = await request.post(
    `${apiBaseUrl}/api/chat-rooms/${chatRoom.id}/leave`  // joiner 세션
  );
...
test('기존 채팅방 입장 시 시스템 메시지 표시 확인', async ({ page, request }) => {
  ...
  const creator = await createUser(request, creatorNickname);
  await createChatRoom(request, roomName);               // creator 세션으로 생성
  const joiner = await createUser(request, joinerNickname);  // 이후 세션 = joiner
...
test('2명 참여 시 상대 읽음 처리 후 미읽음 카운트 소멸', async ({ page, request }) => {
  ...
  const creator = await createUser(request, creatorNickname);
  const chatRoom = await createChatRoom(request, roomName);  // creator 세션으로 생성
  const reader = await createUser(request, readerNickname);  // 이후 세션 = reader

  await joinChatRoom(request, chatRoom.id);  // reader 세션으로 입장
  ...
  // reader 세션이 유지된 상태에서 읽음 처리 (마지막으로 createUser 호출된 사용자 = reader)
  const markReadResponse = await request.post(
    `${apiBaseUrl}/api/read-status/mark-read`,
    { params: { chatRoomId: chatRoom.id } }
  );
...
test('3명 참여 시 1명 읽음 후 미읽음 카운트 1 감소', async ({ page, request }) => {
  ...
  const creator = await createUser(request, creatorNickname);
  const chatRoom = await createChatRoom(request, roomName);    // creator 세션으로 생성
  const readerOne = await createUser(request, readerOneNickname);  // 세션 = readerOne
  await joinChatRoom(request, chatRoom.id);                        // readerOne 입장
  const readerTwo = await createUser(request, readerTwoNickname);  // 세션 = readerTwo
  await joinChatRoom(request, chatRoom.id);                        // readerTwo 입장
  ...
  // readerOne 세션으로 재로그인 후 읽음 처리
  await createUser(request, readerOneNickname);
  const markReadResponse = await request.post(
    `${apiBaseUrl}/api/read-status/mark-read`,
    { params: { chatRoomId: chatRoom.id } }
  );
...
test('아바타 그룹: 본인 제외 활성 멤버 수에 따라 정확한 개수의 아바타 렌더링', async ({ page, request }) => {
  ...
  // 방 ID 파악 (URL에서 추출) — createUser 호출 전에 먼저 저장
  const roomId = page.url().match(/\/(\d+)/)?.[1];
  if (!roomId) throw new Error('방 ID를 파악할 수 없습니다.');

  // 멤버 2명 추가 입장 (API) — 각 createUser 후 해당 세션으로 joinChatRoom
  await createUser(request, `member1_${Date.now()}`);
  await joinChatRoom(request, roomId);
  await createUser(request, `member2_${Date.now()}`);
  await joinChatRoom(request, roomId);
  ...
  // 멤버 3번 추가 → 총 본인 + 3명 = 4명 → 헤더 아바타 3개 (본인 제외)
  await createUser(request, `member3_${Date.now()}`);
  await joinChatRoom(request, roomId);
...
test('아바타 이니셜: 프로필 이미지 없는 멤버는 닉네임 첫 글자 표시', async ({ page, request }) => {
  ...
  const roomId = page.url().match(/\/(\d+)/)?.[1];
  if (!roomId) throw new Error('방 ID를 파악할 수 없습니다.');
  await createUser(request, memberNickname);
  await joinChatRoom(request, roomId);
...
test('내 메시지는 우측 정렬 확인', async ({ page, request }) => {
  ...
  const chatRoom = await createChatRoom(request, roomName);  // otherUser 세션으로 생성
...
test('참가자 필터링 시 해당 참가자의 메시지만 표시', async ({ page, request }) => {
  ...
  await createUser(request, user1Nickname);                            // 세션 = user1
  const chatRoom = await createChatRoom(request, roomName);           // user1 세션으로 생성
  await createUser(request, user2Nickname);                           // 세션 = user2
  await joinChatRoom(request, chatRoom.id);                           // user2 세션으로 입장
