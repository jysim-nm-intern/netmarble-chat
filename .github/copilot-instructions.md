...
- **SQL Injection** — TypeORM 사용 시 Raw Query에서 파라미터 바인딩 누락 여부
...
| 영역 | 스택 |
|------|------|
| Backend | Node.js 20, NestJS 10.x, TypeORM, Mongoose, MySQL, MongoDB, Redis, STOMP |
| Frontend | React (Vite), Zustand, TailwindCSS, @stomp/stompjs, SockJS |
| Test | Jest + @nestjs/testing (Server), Vitest + Testing Library (Client), Playwright (E2E) |
...
Domain (Model + Repository Interface)
    ↑  구현체 주입
Infrastructure (TypeORM Repository 구현, WebSocket Config 등)
```

**계층 규칙:**
- Controller는 Repository를 직접 호출하지 않습니다.
- Domain 모델(Entity)을 API 응답으로 직접 반환하지 않습니다. 반드시 DTO를 사용합니다.
- Domain 로직은 외부 기술(TypeORM, Mongoose, STOMP, Kafka 등)에 의존하지 않습니다.
...
- [ ] `class-validator` 데코레이터 또는 도메인 생성자 내 검증 로직이 존재하는가

### 4. 예외 처리

- [ ] 비즈니스 예외가 `GlobalExceptionFilter`를 통해 일관된 형식으로 응답되는가
...
- [ ] 통합 테스트는 `*.e2e-spec.ts` 파일로 분리되어 있는가
...
**모든 PR 제목은 한국어로 작성합니다.** 영어 제목은 절대 사용하지 않습니다.
...
예시:
```
[Phase 1] 아바타 렌더링 단위 테스트 추가 (#28)
[Phase 1] 혼자 있는 채팅방 헤더 아바타 표시 수정 (#28)
```

### "Implement suggestion"으로 PR을 생성할 때 (코드 리뷰 제안 반영)

Copilot이 PR 리뷰 코멘트의 "Implement suggestion"을 통해 새 PR을 생성하는 경우, 아래 규칙을 **반드시** 따릅니다.

**PR 제목:**
```
[Phase N] 한국어 제목 (from #원본PR번호 Copilot 제안)
```

예시:
```
[Phase 1] ChatRoomList 아바타 폴백 단위 테스트 추가 (from #28 Copilot 제안)
```

**PR 본문 — `## 📝 변경 사항 요약` 에 원본 제안 출처를 반드시 명시:**

```markdown
## 🔗 관련 이슈 / SPEC

- 원본 PR: #N (Copilot 코드 리뷰 제안 반영)
- 관련 SPEC: `SPEC-XXX-000` (해당하는 경우)
...
### 원본 제안 출처
- PR #N Copilot 리뷰 코멘트 — "{제안 핵심 내용 한국어 요약}"
- 제안 유형: `[권고]` / `[필수]` / `[제안]`
```
