...
| `Unauthorized` | 401 | 로그인 세션 없이 보호 엔드포인트 접근 |

> **인증 방식:** `POST /api/users` 로그인 성공 시 서버가 `JSESSIONID` 세션 쿠키를 발급한다.
> 이후 모든 보호 엔드포인트는 이 쿠키로 인증된 사용자 정보를 추출하며,
> 클라이언트가 전달하는 userId 파라미터는 세션 값으로 서버가 대체한다.
...
| **성공 응답** | `201 Created` · `{ "userId": 1, "nickname": "string", "profileColor": "#hex", "profileImage": "data:image/...;base64,..." }` · `Set-Cookie: JSESSIONID=...` |
| **실패 응답** | `400` · `INVALID_NICKNAME` (형식 오류) |
| **비즈니스 규칙** | 닉네임 중복 시 기존 사용자 반환하되 profileColor·profileImage 갱신. 응답 쿠키(`JSESSIONID`)에 세션 발급 |
...
| **부수 효과** | 서버 세션 무효화. 세션 없이 호출해도 204 반환 |
...
#### POST /api/chat-rooms — 채팅방 생성 🔒 인증 필요 (JSESSIONID 쿠키)

| 항목 | 내용 |
|------|------|
| **Content-Type** | `multipart/form-data` |
| **요청 파라미터** | `name` (String, 필수, 2자 이상) · `image` (MultipartFile, 선택, JPG/PNG/GIF, 최대 5MB) |
| **성공 응답** | `201 Created` · `{ "roomId": 1, "name": "string", "imageUrl": "data:image/png;base64,...", "isActive": true, "createdAt": "..." }` |
| **실패 응답** | `400` · `INVALID_ROOM_NAME` / `401` · 세션 없음 |
| **부수 효과** | SYSTEM 메시지 생성 + WebSocket 브로드캐스트 |
| **비즈니스 규칙** | 생성자 ID는 클라이언트 파라미터가 아닌 서버 세션에서 결정 |

#### POST /api/chat-rooms/{roomId}/join — 채팅방 입장 🔒 인증 필요 (JSESSIONID 쿠키)

| 항목 | 내용 |
|------|------|
| **Path 파라미터** | `roomId: Long` |
| **요청 Body** | 없음 (참여자 ID는 세션에서 서버가 결정) |
| **성공 응답** | `200 OK` · 아래 |
| **실패 응답** | `404` · `ROOM_NOT_FOUND` / `401` · 세션 없음 |
| **부수 효과** | SYSTEM 메시지 생성 + WebSocket 브로드캐스트 |
...
#### POST /api/chat-rooms/{roomId}/leave — 채팅방 퇴장 🔒 인증 필요 (JSESSIONID 쿠키)

| 항목 | 내용 |
|------|------|
| **Path 파라미터** | `roomId: Long` |
| **요청 Body** | 없음 (퇴장자 ID는 세션에서 서버가 결정) |
...
#### /app/chat.sendMessage — 메시지 전송 🔒 인증 필요 (STOMP CONNECT 시 세션 검증)
...
| 필드 | 타입 | 필수 | 설명 |
|------|------|------|------|
| `chatRoomId` | Long | O | 대상 채팅방 ID |
| `messageType` | String | O | `TEXT` / `IMAGE` / `STICKER` |
| `content` | String | O | 메시지 본문 |

> **보안:** 발신자 ID는 클라이언트 payload에 포함하지 않는다.
> 서버가 STOMP 세션 attributes에서 `userId`를 추출하여 발신자로 설정하므로
> 클라이언트가 userId를 임의로 조작해도 실제 세션 사용자로 저장된다.

#### /app/chat.read — 읽음 처리 🔒 인증 필요
