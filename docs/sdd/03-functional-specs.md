...
**설명:** 로그인된 사용자는 활성화된 모든 채팅방 목록과 각 방의 미읽음 메시지 수를 확인할 수 있다. 참가중인 채팅방의 마지막 메시지와 미읽음 수는 WebSocket 구독을 통해 실시간으로 갱신된다.
...
- **AC-ROOM-001-9:** 다른 사용자가 참가중인 채팅방에 메시지를 전송하면, 페이지 새로고침 없이 해당 채팅방의 마지막 메시지 내용(`lastMessageContent`)과 시간(`lastMessageAt`)이 실시간으로 갱신된다.
- **AC-ROOM-001-10:** 다른 사용자의 새 메시지 수신 시, 해당 채팅방의 `unreadCount`가 실시간으로 1 증가하여 배지에 반영된다. 본인이 전송한 메시지와 시스템 메시지는 `unreadCount`에 영향을 주지 않는다.
- **AC-ROOM-001-11:** 실시간 목록 갱신은 사용자의 활성 상태(active/inactive)와 무관하게 WebSocket 연결이 유지되는 한 항상 동작한다.
- **AC-ROOM-001-12:** 이미지 메시지 수신 시 마지막 메시지 미리보기에 `[사진]`, 스티커 수신 시 `[스티커]`로 표시된다.
