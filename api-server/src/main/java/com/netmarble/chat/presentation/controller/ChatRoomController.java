...
public class ChatRoomController {
    ...
    /**
     * Cursor-based 메시지 이력 조회 (MongoDB)
     * GET /api/chat-rooms/{id}/messages?cursor={lastMessageId}&limit=50&direction=BEFORE
     *
     * @param id        채팅방 ID
     * @param cursor    마지막 메시지 ObjectId (없으면 최신부터)
     * @param limit     조회 건수 (기본 50, 최대 100)
     * @param direction BEFORE(이전 메시지) | AFTER(이후 메시지)
     */
    ...
    public ResponseEntity<CursorPageResponse<MessageResponse>> getMessages(
        ...
        log.info("GET /api/chat-rooms/{}/messages cursor={}, limit={}, direction={}", id, cursor, limit, direction);
        CursorPageResponse<MessageResponse> response = messageQueryService.findByCursor(id, cursor, limit, direction);
        return ResponseEntity.ok(response);
    }
}
