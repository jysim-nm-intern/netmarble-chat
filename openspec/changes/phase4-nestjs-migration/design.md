...
1. 기존 서버 폴더 리네이밍: `chat-server` → `server/chat-server-spring`, `api-server` → `server/api-server-spring`
...
### Rollback 전략
기존 Java 서버가 `-spring` 접미사로 보존되므로, docker-compose.yml의 빌드 컨텍스트를 `server/chat-server-spring`, `server/api-server-spring`으로 변경하면 즉시 롤백 가능.
